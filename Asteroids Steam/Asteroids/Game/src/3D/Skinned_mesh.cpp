...
bool SkinnedMesh::InitFromScene(const aiScene* pScene, const std::string& Filename)
{
	...
	// Без этой строки не получается вывод GUI(2D menu) и 3D совместно, через SFML!
...
void SkinnedMesh::LoadBones(uint MeshIndex, const aiMesh* pMesh, std::vector<VertexBoneData>& Bones)
{
	...
	// made in idris070 * //
	// Если объект без костей, имитирует кости! //
		...
		if (pMaterial->GetTextureCount(aiTextureType_DIFFUSE) > 0) {
			...
			if (pMaterial->GetTexture(aiTextureType_DIFFUSE, 0, &Path, NULL, NULL, NULL, NULL, NULL) == AI_SUCCESS) {
				...
				if (p[0] == '*') {
					...
					int numT = atoi(p.c_str());
					m_Textures[i]->loadFromMemory(pScene->mTextures[numT]->pcData, pScene->mTextures[numT]->mWidth);
					break;
				}
...
uint SkinnedMesh::FindScaling(float AnimationTime, const aiNodeAnim* pNodeAnim)
{
	...
	// AnimationTime оказывается больше последнего значения на маленькое значение!
	assert(0);
...
uint SkinnedMesh::GetNumAnimation()
{
