...
public:
	...
	INLINE explicit Tquat(const Tvec3<T> &v):x(v.x),y(v.y),z(v.z),w(0) {}

	INLINE explicit Tquat(const Tmat3<T> &M,HMbool quatIsUnit=HMtrue)
	{
...
/*		Tquat<T> q,q2;
		...
		if (equal(M[0].x,T(-1))) {q.xyz()=Tvec3<T>(0,1,0); q.w=0;}
		else
		{
			q.xyz()=cross(Tvec3<T>(1,0,0),M[0]+Tvec3<T>(1,0,0));
			q.w=dot(Tvec3<T>(1,0,0),M[0]+Tvec3<T>(1,0,0));
		}
		//Quaternion q superpose X-axis of UCS with axis M[0], i.e. it rotates UCS to (tx,ty,tz), where tx=M[0]
		Tvec3<T> ty=normalize(q*Tvec3<T>(0,1,0));
		//Now we should superpose ty with M[1]
		if (ty==-M[1]) {q2.xyz()=Tvec3<T>(1,0,0); q2.w=0;}
		else
		{
			q2.xyz()=Tvec3<T>(-1,0,0)*dot(M[2],ty+M[1]);
			q2.w=dot(M[1],ty+M[1]);
		}
...
/*/		//REMARK: Code was taken from game engine "nebuladevice"
	...
	INLINE explicit Tquat(const Tmat4<T> &M,HMbool quatIsUnit=HMtrue) {*this=Tquat(Tmat3<T>(M),quatIsUnit);}

	INLINE Tquat(const Tvec3<T> &v,const T alpha)
	{
		T len=length(v);
		if (notZero(len))
		{
			xyz()=v*(sin(alpha/T(2))/len);
			w=cos(alpha/T(2));
		}
		else
		{
	...
	INLINE vec3 &xyz() {return *(vec3*)&x;}
	INLINE const vec3 &xyz() const {return *(const vec3*)&x;}
...
#ifdef QUAT_MUL_REVERSE_ORDER
template <class T>
INLINE const Tvec3<T> operator*(const Tvec3<T> &v,const Tquat<T> &q)
{return (inverse(q)*(Tquat<T>(v)*q)).xyz();}
#endif

template <class T>
INLINE const Tquat<T> operator*(const T a,const Tquat<T> &q) {return Tquat<T>(a*q.x,a*q.y,a*q.z,a*q.w);}

template <class T> INLINE Tmat3<T>::Tmat3(const Tquat<T> &q,HMbool quatIsUnit)
{
	T k=(quatIsUnit ? 2 : T(2)/norm(q));
	T x2=k*q.x,
	  y2=k*q.y,
	  z2=k*q.z;
