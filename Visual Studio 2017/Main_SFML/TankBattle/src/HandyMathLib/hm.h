...
Handy Math Library
Copyright (C) 2005  Александр Третьяк (tav86@mail.ru)
...
Векторизация для 4-хмерных векторов по умолчанию включена.
...
#define QUAT_MUL_REVERSE_ORDER   Если эта константа определена, умножения двух кватернионов и
                                 ...
                                 правый - кватернионом.
#define DISABLE_VECTORIZATION    Выключает векторизацию для типов Tvec4.
...
TmatN - матрица, где T (может быть 'i','d' или 'ld') и N имеют тот же смысл, что и в TvecN.
...
матрицы, а остальные элементы устанавливаются в 0.0. Если в конструктор кватерниона передаётся
...
mat3(quat q,                    Создаёт 3D-матрицу из кватерниона q. Если quatIsUnit==true,
     bool quatIsUnit=false)     q должен быть единичным кватернионом.
...
quat(mat3 M,                    Создаёт кватернион из 3D-матрицы M. Если quatIsUnit==true, создаётся
     bool quatIsUnit=false)     единичный кватернион. Матрица должна содержать только вращение для
...
quat(vec3 v,float alpha)        Создаёт кватернион вращения вокруг вектора v на alpha радиан.
                                Вектор v не обязан быть единичным.
...
Операции над боксами и прямоугольниками:
  ...
  Сложение (+) двух боксов/прямоугольников даёт объединение операндов, т.е. такой бокс/прямоуголь-
ник, который содержит оба операнда. Умножение (*) двух боксов/прямоугольников даёт пересечение
операндов, результат которого имеет тот же тип, что и операнды. Сложение (+) бокса/прямоугольника и
3D/2D-вектора расширяет бокс/прямоугольник таким образом, чтобы он содержал этот вектор,
результатом операции является полученный бокс/прямоугольник.
...
genType inversesqrt(genType x)            Возвращает обратный квадратный корень числа x.
                                          Результат неопределён если x <= 0.
...
genType clamp(genType x,                  Возвращает min (max (x, minVal), maxVal).
              genType minVal,
              genType maxVal)
...
bool intersect(rect b0,rect b1)           возвращает false.
...
float norm(quat q)                        Возвращает норму кватерниона q.
float magnitude(quat q)                   Возвращает модуль (длину) кватерниона q.
float length(quat q)                      То же что и magnitude(q).
...
quat shortestArcQuat(vec3 from,           Возвращает кватернион, который вращает вектор from к
                     vec3 to)             вектору to по кратчайшему пути.

quat shortestArcUnitQuat(vec3 from,       Возвращает единичный кватернион, который вращает вектор
                         vec3 to)         from к вектору to по кратчайшему пути.

mat3 quatToMat (quat q)                   Преобразует кватернион q в 3D матрицу вращения и
mat3 quatToMat3(quat q)                   возвращает результат.

mat4 quatToMat4(quat q)                   Преобразует кватернион q в 4D матрицу вращения и
                                          возвращает результат.

mat3 unitQuatToMat (quat q)               Преобразует единичный кватернион q в 3D матрицу вращения
mat3 unitQuatToMat3(quat q)               и возвращает результат.

mat4 unitQuatToMat4(quat q)               Преобразует единичный кватернион q в 4D матрицу вращения
                                          и возвращает результат.

float quatToAxisAngle(out vec3 axis,      Функция возвращает угол поворота кватерниона q и в axis
                      quat q)             помещает единичный вектор-ось, вокруг которого он
                                          производит вращение.

float unitQuatToAxisAngle(out vec3 axis,  Функция возвращает угол поворота единичного кватерниона q
...
vec3 quatToAxisAngle(quat q)              Функция возвращает вектор-ось, вокруг которого производит
                                          вращение кватернион q. Длина этого вектора равна углу
                                          вращения в радианах.

vec3 unitQuatToAxisAngle(quat q)          Функция возвращает вектор-ось, вокруг которого производит
                                          вращение единичный кватернион q. Длина этого вектора
                                          равна углу вращения в радианах.

float quatAngle(quat q)                   Возвращает угол вращения кватерниона q в интервале
                                          [0; 2*PI).

float unitQuatAngle(quat q)               Возвращает угол вращения единичного кватерниона q в
                                          интервале [0; 2*PI).

quat matToQuat(mat M)                     Преобразует матрицу M в кватернион и возвращает
                                          результат.

quat matToUnitQuat(mat M)                 Преобразует матрицу M в единичный кватернион и возвращает
                                          результат.
