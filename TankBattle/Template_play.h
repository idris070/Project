...
//std::list<Entity*>::iterator it3;//второй итератор.для взаимодействия между объектами списка
//
...
//	Clock Time;
//	float timeS;
//	Bullet(Image &image, float X, float Y, int W, int H, float Rotation) :Entity(image,X,Y,W,H){//всё так же, только взяли в конце состояние игрока (int dir)
...
//			entities.push_back(new Bullet(BulletImage, x+(bulletSin*56/2), y-(bulletCos*56/2), 16, 16, sprite2.getRotation()));
//
//			randX = 1 + rand() % 1300 - x;
//			randY = 1 + rand() % 1000 - y;
//			resetMove.restart();
...
//	Clock Time;
//	float timeS;
//	Track(Image &image, float X, float Y, int W, int H, float Rotation) :Entity(image,X,Y,W,H){//всё так же, только взяли в конце состояние игрока (int dir)
//		x = X;
//		y = Y;
//		rotation = Rotation;
//		life = true;
...
//	//	{
//	//		Entity *b = *it3;//для удобства, чтобы не писать (*it)->
//	//		if(b->name=="Enemy"){
//	//			entenemy.push_back(*it3);
//	//		}
//	//		it3++;
...
//				entities.push_back(new Bullet(BulletImage, p.x+(bulletSin*56/2), p.y-(bulletCos*56/2), 16, 16, p.sprite2.getRotation())); //и передаем в конструктор пули. создается пуля
//				entenemy.push_back(entities.back());
